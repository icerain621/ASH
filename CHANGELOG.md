...
- Sprint EW14（W1 v6.0 吸收）：`internal/execpolicy` — 声明式 `ash.execpolicy.v1`（network/fs/process 能力地板）、SpacePolicy `bodyJson.execPolicy` 与 Harness Spec 合成解析、`Merge` 更严者优先（V6-D3）；附录 [`doc/appendices/ash-execpolicy-v1.md`](doc/appendices/ash-execpolicy-v1.md)。尚未接入 sandbox Resolve（→ EW15）。
...
- Sprint DX11（v2.3）：范围冻结 + 签字门禁 — `v2.3-release-scope` 已冻结；`make v2.3-signoff`（含 `rag-hybrid-smoke`）；清单/签字模板；DX9 Minor 收口（digest 索引、prefer=path、lane 错误传播、rebuild API 测）；**不自动**打 `v2.3.0` tag。
- Sprint DX10（v2.3）：Hybrid 接线 — Run prepare `RebuildSymbols` 事件；`ash rag rebuild`；Knowledge/Observability/Scale Hybrid 面板；Scale readiness hybrid 字段。
- Sprint DX9（v2.3）：RAG Hybrid 符号索引（方案 C）— 表 `rag_path_entries` / `rag_symbols`（SQL **28** / RLS **48**）；`RebuildSymbols` + Hybrid Query（path/symbol/text RRF）；`POST /rag/symbols/rebuild`；`make rag-hybrid-smoke`。
