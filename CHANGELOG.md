...
- Sprint BP：跨平台 `testutil.WriteFakeExecGoCLI`；`OpenTest` Windows SQLite 清理；CI/Postgres E2E 失败日志 artifact + docker 诊断；补齐 apicodes 缺码；RLS tester 角色改为幂等 ensure；`migrate()` 使用打开时的 owner DSN（勿误用 `ASH_DATABASE_APP_URL`）。
