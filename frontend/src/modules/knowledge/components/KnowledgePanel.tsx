...
type KnowledgePanelProps = {
  /** When true, omit outer page chrome so the panel can nest under Memory tabs. */
  embedded?: boolean;
};

export function KnowledgePanel({ embedded = false }: KnowledgePanelProps) {
  ...
  const toolbar = (
    <div className="toolbar metrics-toolbar" style={embedded ? { marginBottom: "1rem" } : undefined}>
        ...
        <>
          ...
          <div className="page-heading">
            <div>
              <h1>知识中心</h1>
              <p>
                即时 Repo Profile 与 Wiki 投影（不落库）；Run 准备阶段可注入 profile: / wiki: contextRefs，并触发
                Hybrid 符号重建。
              </p>
            </div>
            {toolbar}
        ...
        <div>
          ...
          {profileQuery.data && (
            <div className="card-like" data-testid="knowledge-profile">
              ...
              <p>语言：{(profileQuery.data.languages || []).join(", ") || "—"}</p>
              <p>测试：{(profileQuery.data.testCommands || []).join(" · ") || "—"}</p>
              <p>模块：{(profileQuery.data.modules || []).join(", ") || "—"}</p>
            </div>
          )}
  ...
  if (embedded) {
    return <div data-testid="knowledge-panel">{content}</div>;
  }

  return (
    <section className="panel active" data-testid="knowledge-panel">
      {content}
    </section>
  );
}
