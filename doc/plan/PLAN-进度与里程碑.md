...
**MVP 功能与自动化门禁已完成**；v2 DH–DV 已收口；**v2.1（DW–DX5）** / **v2.2（DX6–DX8）** / **v2.3（DX9–DX11）已冻结**（`make v2.1-signoff` / `v2.2-signoff` / `v2.3-signoff`；tag 人工）。**v2.4 DX12 进行中**（Waker duties；SQL **29** / RLS **50** · `make waker-smoke`；范围**草案 / 未冻结**）。当前并行线：**生产环境验收**（云 RDS / 真实 CI·ExecGo）与 tag 签字。
