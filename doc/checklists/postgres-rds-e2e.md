...
| ALL | 36 | **全部 pass**（含 TR0–TR3、M2、**TR3-05** 指标回放一致性、**TR3-06** Postgres FTS、**TR3-07** 插件导出健康） |
...
| 7.2 | SSE | `GET /runs/{id}/stream` | 事件流正常；审计 `stream.session_*` |
...
- [ ] `doctor --suite ALL` 报告（36/36 pass）
- [ ] RLS 集成测试日志
- [ ] `readyz` 响应 + 切换时间戳
- [ ] §7 业务抽样记录
