...
# 期望：M3 11/11，ALL 42/42（M3-09 readyz 契约，M3-10 RLS 全局表，M3-11 RLS 目录，TR3-05..10）
```
...
| GitHub `GITHUB_TOKEN`（CI sync） | Console Secrets 更新 → 不改明文连接配置 | `ASH_CI_FIXTURE=1` 本地联调；生产 `GET /ci/runs?sync=true` |
